...
/*   Updated: 2023/05/20 16:17:13 by makurz           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	rotate_inputs(t_fdf *fdf)
{
	...
	shift_mod = mlx_is_key_down(fdf->mlx, MLX_KEY_LEFT_SHIFT);
	if (shift_mod && mlx_is_key_down(fdf->mlx, MLX_KEY_A))
		anti_rotate(MLX_KEY_A, fdf);
	else if (mlx_is_key_down(fdf->mlx, MLX_KEY_A))
		rotate(MLX_KEY_A, fdf);
	if (shift_mod && mlx_is_key_down(fdf->mlx, MLX_KEY_S))
		anti_rotate(MLX_KEY_S, fdf);
	else if (mlx_is_key_down(fdf->mlx, MLX_KEY_S))
		rotate(MLX_KEY_S, fdf);
	if (shift_mod && mlx_is_key_down(fdf->mlx, MLX_KEY_D))
